...
def get_next_step_from_fodf(fodf, streamlines, step, sphere, step_size, tracker):
    ...
    most_probable_directions = torch.argmax(fodf, dim=1)
    ...
    streamlines_clone = streamlines.clone()
    streamlines_clone[:, step+1, :] = next_positions
    # Check if any of the manual stopping criteria are met
    terminated_streamlines = calc_stopping_mask(tracker, streamlines_clone, step+1) | model_termination_decisions
...
def calc_stopping_mask(tracker, streamlines, step):
    """
    ...
    stop_criteria_met |= outside_image_bounds(current_voxels, tracker.fa_map.shape)
